...
- CMS 预览组件：`src/app/components/landing/*`、`src/app/contexts/*`、`src/app/translations/translations.ts`，并补充本项目缺失的 `src/app/components/ui/carousel.tsx` 与 `src/app/components/ui/dialog.tsx`；
...
- 内容替换策略：保留 `cms/source` 原有页面结构、分区、Puck 预览和可编辑字段，直接更新 `data/cms.db` 的 `site_state.site_content_json` 与 `page_content_json`，将默认导航、首页、About、荣誉、动态、服务行业、团队和联系字段替换为虎诉官网内容；同时更新 `src/lib/site-settings.ts` 默认站点设置，避免数据库重建时回退到旧个人站导航。
- 旧内容清理：`data/cms.db` 的当前工作区与历史版本快照、`src/app/components/landing/*`、`src/app/translations/translations.ts`、`src/lib/cms-page-content.ts` 中的 Dennis / Daxuange / Zhong-Er / Tiger Legal Talks 等旧个人站默认文案已清零替换，避免 CMS 预览缺字段或恢复版本时回退到旧站内容。
